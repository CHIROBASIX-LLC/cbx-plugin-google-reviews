...
/**
 * GitHub-based auto-updater for the CHIROBASIX Google Reviews Widget plugin.
 */
class CBXR_Updater {
	...
	private function get_github_release( $allow_network = true ) {
		...
		if ( ! $allow_network || false !== get_transient( 'cbxr_github_backoff' ) ) {
		...
		$response = wp_remote_get( $url, array(
			'headers' => array(
				...
				'User-Agent' => 'CBXR-Google-Reviews-Updater',
			),
			'timeout' => 3,
		));

		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			set_transient( 'cbxr_github_backoff', 1, 15 * MINUTE_IN_SECONDS );
	...
	public function plugin_info( $result, $action, $args ) {
		...
		$info              = new stdClass();
		$info->name        = 'CHIROBASIX Google Reviews Widget';
		...
		$info->sections    = array(
			'description' => 'Displays Google Reviews as a floating widget with slide-out panel.',
